...
class NodePosition():
    ...
    def initParameters(self):
        ...
        self.xe_prev = 0
        self.v_limit = 0.5
        self.xd=3*np.cos(np.linspace(0,2*np.pi,len(self.t)+1))
        self.yd=3*np.sin(np.linspace(0,2*np.pi,len(self.t)+1))
    ...
    def controller(self):
        ...
        self.xe = self.xd[self.i+1] - self.xr
        self.ye = self.yd[self.i+1] - self.yr
        if self.xe<0.5 and self.ye<0.5 and self.i<len(self.t)-1:
            self.i=self.i+1
            print('paso: '+str(self.xe)+' ,'+str(self.ye))
